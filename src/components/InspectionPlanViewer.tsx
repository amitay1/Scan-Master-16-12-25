...
function drawProfessionalCrossSection(
  ...
  switch (partType) {
    ...
    // PLATE/SHEET - Thin flat cross-sections (W/T > 5)
    case 'plate':
    case 'sheet':
      drawPlateCrossSection(ctx, centerX, centerY, bounds, dimensions);
      break;

    // BOX/BAR/BLOCK - Rectangular cross-sections (thicker proportions)
    case 'bar':
    case 'box':
    case 'slab':
    case 'flat_bar':
    ...
    case 'rectangular_forging_stock':
      drawBoxCrossSection(ctx, centerX, centerY, bounds, dimensions);
      break;
    ...
    case 'bar': {
      ...
      switch (direction.toUpperCase()) {
        case 'A': // E2375 Fig.6: Primary - from top through thickness
          return {
            x: centerX + 25, y: centerY - plateH - 30,
            symbolX: centerX + 25, symbolY: centerY - plateH - 50,
            labelX: centerX + 50, labelY: centerY - plateH - 35,
        ...
        case 'B': // E2375 Fig.6: Secondary - from adjacent side (required if W/T<5)
          return {
            x: centerX - plateW - 30, y: centerY - 25,
            symbolX: centerX - plateW - 50, symbolY: centerY - 25,
            labelX: centerX - plateW - 45, labelY: centerY - 45,
        ...
        case 'B₁': // Dual Element - Near-surface 0-20mm from side
          return {
            x: centerX - plateW - 30, y: centerY + 25,
            symbolX: centerX - plateW - 50, symbolY: centerY + 25,
            labelX: centerX - plateW - 45, labelY: centerY + 5,
            arrowDirection: 'right', isAngleBeam: false
          };
        case 'C': // Opposite side (required if >9 inches)
          return {
            x: centerX + 25, y: centerY + plateH + 30,
            symbolX: centerX + 25, symbolY: centerY + plateH + 50,
            labelX: centerX + 50, labelY: centerY + plateH + 45,
    ...
    case 'round_forging_stock': {
      ...
      switch (direction.toUpperCase()) {
        case 'A': // E2375 Fig.6: Radial from top (part rotates)
          return {
            x: centerX + 20, y: centerY - barRadius - 35,
            symbolX: centerX + 20, symbolY: centerY - barRadius - 55,
            labelX: centerX + 50, labelY: centerY - barRadius - 40,
            arrowDirection: 'down', isAngleBeam: false
          };
        case 'A₁': // Dual Element - Near-surface 0-20mm from top
          return {
        ...
        case 'B': // E2375 Fig.6: Radial from side (alternative position)
          return {
            x: centerX - barRadius - 35, y: centerY - 20,
            symbolX: centerX - barRadius - 55, symbolY: centerY - 20,
            labelX: centerX - barRadius - 50, labelY: centerY - 40,
            arrowDirection: 'right', isAngleBeam: false
          };
        case 'B₁': // Dual Element - Near-surface 0-20mm from side
          return {
            x: centerX - barRadius - 35, y: centerY + 20,
            symbolX: centerX - barRadius - 55, symbolY: centerY + 20,
            labelX: centerX - barRadius - 50, labelY: centerY,
            arrowDirection: 'right', isAngleBeam: false
          };
    ...
    case 'bushing': {
      ...
      switch (direction.toUpperCase()) {
        case 'A': // Straight beam from top (on side view)
          return {
            x: sideViewX + 20, y: centerY - tubeRadius - 35,
            symbolX: sideViewX + 20, symbolY: centerY - tubeRadius - 55,
            labelX: sideViewX + 50, labelY: centerY - tubeRadius - 40,
            arrowDirection: 'down', isAngleBeam: false
          };
        case 'A₁': // Dual Element - Near-surface 0-20mm from top
          return {
        ...
        case 'B': // Straight beam from left end (on side view)
          return {
            x: sideViewX - sideWidth / 2 - 35, y: centerY - 20,
            symbolX: sideViewX - sideWidth / 2 - 55, symbolY: centerY - 20,
            labelX: sideViewX - sideWidth / 2 - 60, labelY: centerY - 40,
            arrowDirection: 'right', isAngleBeam: false
          };
        case 'B₁': // Dual Element - Near-surface 0-20mm from left end
          return {
            x: sideViewX - sideWidth / 2 - 35, y: centerY + 20,
            symbolX: sideViewX - sideWidth / 2 - 55, symbolY: centerY + 20,
            labelX: sideViewX - sideWidth / 2 - 60, labelY: centerY,
            arrowDirection: 'right', isAngleBeam: false
          };
...
function drawPlateCrossSection(
  ...
  // Get dimensions - PLATE must be thin (W/T > 5 per ASTM E2375-16)
  const rawThickness = dimensions.thickness || dimensions.height || 30;
  const width = dimensions.width || dimensions.length || 200;
  const depth = dimensions.depth || width * 0.6; // Deeper for plate visualization
  
  // For PLATE: enforce thin appearance - thickness should be much smaller than width
  // If ratio is less than 5, adjust thickness visually to look like a plate
  const widthToThicknessRatio = width / rawThickness;
  const thickness = widthToThicknessRatio < 5 ? width / 8 : rawThickness; // Force plate-like appearance

  // Scale to fill canvas (70% of available space)
  const maxSize = Math.min(bounds.width, bounds.height) * 0.7;
  // Scale based on width and depth, thickness is thin
  const scale = maxSize / Math.max(width, depth);

  const w = width * scale;
  const h = Math.max(thickness * scale, 8); // Minimum visible thickness but keep it thin
...
/**
 * Draw PROFESSIONAL CAD-GRADE 3D BOX/BAR
 * For rectangular parts with thicker proportions (not plates)
 * Features:
 * - Isometric 3D visualization
 * - Gradient fills for 3D depth perception
 * - 45-degree ISO hatching on front face
 * - Uses 70% of available canvas space
 */
function drawBoxCrossSection(
  ctx: CanvasRenderingContext2D,
  centerX: number,
  centerY: number,
  bounds: DrawBounds,
  ...
  // Get dimensions - preserve actual proportions for box/bar
  ...
  ctx.font = 'bold 16px Arial';
  ctx.textAlign = 'center';
  ctx.fillText('BOX', centerX, y + h + 50);

  // Underline
  ctx.strokeStyle = '#1a1a2e';
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(centerX - 25, y + h + 55);
  ctx.lineTo(centerX + 25, y + h + 55);
  ctx.stroke();
