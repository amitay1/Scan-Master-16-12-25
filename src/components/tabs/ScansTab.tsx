...
export const ScansTab = ({ scans, onChange }: ScansTabProps) => {
  ...
  const removeImage = (scanId: string, field: 'cScanImage' | 'aScanImage') => {
    updateScan(scanId, field, undefined);
  };
          ...
          <Card key={scan.id} className="p-4">
            ...
            {expandedScan === scan.id && (
              <div className="space-y-4 border-t pt-4">
                ...
                {/* Image Uploads - Real scan images only */}
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label>C-Scan Image</Label>
                    <p className="text-xs text-muted-foreground">
                      Upload actual C-Scan image from your UT equipment
                    </p>
                    <div className="flex items-center gap-2">
                      ...
                      {scan.cScanImage && (
                        <Button
                          ...
                          variant="ghost"
                          size="sm"
                          onClick={() => removeImage(scan.id, 'cScanImage')}
                        >
                          <X className="h-4 w-4" />
                        </Button>
                      )}
                    ...
                    {scan.cScanImage ? (
                      ...
                      <div className="w-full h-32 border-2 border-dashed rounded flex items-center justify-center text-muted-foreground text-sm">
                        No C-Scan image uploaded
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Upload actual A-Scan waveform from your UT equipment
                    </p>
                    <div className="flex items-center gap-2">
                      <Button
                        type="button"
                      ...
                      {scan.aScanImage && (
                        <Button
                          ...
                          variant="ghost"
                          size="sm"
                          onClick={() => removeImage(scan.id, 'aScanImage')}
                        >
                          <X className="h-4 w-4" />
                        </Button>
                      )}
