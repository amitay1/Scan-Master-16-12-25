...
/**
 * AUTOMATIC SHAPE CLASSIFICATION based on Rafael 5036 Standard
 * 
 ...
 *   - RING: height ≤ 2 × wall thickness (short hollow cylinder)
 *   - TUBE: height > 2 × wall thickness (long hollow cylinder)
 * 
 * For SOLID cylindrical shapes (no inner diameter):
 *   - DISK: height ≤ diameter / 3 (short solid cylinder)
 *   - CYLINDER: height > diameter / 3 (long solid cylinder)
 * 
...
function classifyCircularShape(
  ...
  if (isHollow) {
    ...
    if (calculatedWallThickness > 0) {
      // Rafael 5036: Ring when height ≤ 2 × wall thickness
      const isRing = height <= 2 * calculatedWallThickness;
      ...
      if (isRing) {
        ...
        // It's a Tube (long hollow cylinder)
    ...
    // Rafael 5036: Disk when height ≤ diameter / 3
    const isDisk = height <= diameter / 3;
    ...
    if (isDisk) {
      ...
      // It's a Cylinder (long solid cylinder)
...
export const InspectionSetupTab = ({ data, onChange, acceptanceClass }: InspectionSetupTabProps) => {
  ...
  // Ring ↔ Tube (hollow) and Disk ↔ Cylinder (solid)
  // ═══════════════════════════════════════════════════════════════════════════
  React.useEffect(() => {
    if (!data.partType) return;
    
    // Get height from partLength or partThickness
    const height = data.partLength || data.partThickness;
    
    console.log('🔍 Checking auto-classification:', {
      currentType: data.partType,
    ...
    // Classify the shape based on current dimensions
    const newPartType = classifyCircularShape(
      data.partType,
      data.diameter,
      data.innerDiameter,
      height,
    ...
    console.log(`🔍 Classification result: ${data.partType} → ${newPartType}`);
  ...
  }, [data.partType, data.diameter, data.innerDiameter, data.partLength, data.partThickness, data.wallThickness, onChange]);
